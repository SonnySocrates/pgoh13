...
<meta charset="utf8">
<style>
body {font-family:arial;padding:0 10px;width:100%;max-width:450px;}
...
<script>
    ...
    function afficherHeure() {
        ...
        const heuresMy = maintenant.getHours().toString().padStart(2, '0');
        const minutesMy = maintenant.getMinutes().toString().padStart(2, '0');

        // Remplir les balises <span> avec les valeurs dynamiques
        document.getElementById("heures-My").textContent = heuresFrance;
        document.getElementById("minutes-My").textContent = minutesFrance;
    }
...
const joursMalais = ["Ahad", "Isnin", "Selasa", "Rabu", "Khamis", "Jumaat", "Sabtu"];
        ...
        const jourSemaineMalais = joursMalais[maintenant.getDay()];
        const jourSemaineAnglais = joursAnglais[maintenant.getDay()];
        ...
        // Calcul des heures pour les autres pays
        const nepalTime = new Date(maintenant.getTime() + (4 * 60 + 45) * 60000);
        const malaysiaTime = new Date(maintenant.getTime() + 7 * 60 * 60000);
        const londonTime = new Date(maintenant.getTime() - 1 * 60 * 60000);
        const indiaTime = new Date(maintenant.getTime() + (4 * 60 + 30) * 60000);
        ...
        // Affichage
        const dateHeure = `
        Sekarang pukul ${heuresFrance}:${minutesFrance} (hari ${jourSemaineMalais}) di Perancis = It is now ${heuresFrance}:${minutesFrance} (${jourSemaineAnglais}) in France.
        <br><br>
...
<p><img src=clock_logo1.jpg class="imageflottante" />Winter time in France starts on the last Sunday of October each year and lasts for 5 months (until the last Saturday of March) when it's changed to summer time (also called Daylight Saving Time).<br>To avoid any possible misunderstanding in interpreting the time, the 24-hour time is shown here. But in spoken Malay you are expected to use the 12-hour time and say <font color=red>pagi, tengah hari, petang</font> or <font color=red>malam</font> after it depending on whether it is morning, afternoon, evening or night.
<br>The day, date and time of the various places given here are in relation to the time of my server in France which at the moment is ${jourSemaine} ${jourMois} ${nomMois}
<br>
Telling the time for real (applicable only from last Sunday of October to last Saturday of March)
...
<p>This is a running clock. It will update the time every second. If you prefer a fixed clock (more practical for practice) go further down the page.</p>
<table style="background-color:orange;max-width:700px;"><tr><td style="padding-left:10px;">
...
<hr>
<p>The time here will stay fixed. This is more practical for students because by the time you come up with the right answer, the time might have changed!</p>
<table style="background-color:green;max-width:700px;"><tr><td>
<?
...
<em>It is now <span id="heures-My"></span>:<span id="minutes-My"></span>
 in Malaysia (France +7 hours)</em>';
echo "<br>";
...
<table style="max-width:700px;background-color:white"><tr><td><span style="font-size:0.8em;"><b>A quick revision:</b> hari Ahad (Sunday), Isnin (Monday), Selasa (Tuesday), Rabu (Wednesday), Khamis (Thursday), Jumaat (Friday), Sabtu (Saturday)<br><br>
