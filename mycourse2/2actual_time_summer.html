...
<title>Exercises in telling the time in Malay: Current time in New York, London, Paris, Kuala Lumpur, New Zealand, Hawaii, India</title>
...
<br>
<h1>Exercises in telling the time.....summer time (DST)</h1>
	
<p style="max-width:400px;font-style:italic;font-weight:300;font-size:12px;"><img src=clock_logo1.jpg class="imageflottante" />Summer time (also known as Daylight Saving Time) in France starts on the last Sunday of March each year and lasts for 7 months (until the last Saturday of October) when it's changed to winter time.<br>To avoid any possible misunderstanding in interpreting the time, the 24-hour time is shown here. But in spoken Malay you are expected to use the 12-hour time and say <font color=red>pagi, tengah hari, petang</font> or <font color=red>malam</font> after it depending on whether it is morning, afternoon, evening or night. And, by the way, this is not a running clock. You have to refresh the page to get the latest time!
<br>The day, date and time of the various places given here are in relation to the time of my server in France which at the moment is <?php 
 $t=date("l j F Y, H:i") ;
echo $t ; ?>
</p>
<br>
<table style="background-color:yellow;max-width:400px;"><tr><td><b>Telling the time for real (applicable only from last Sunday of March to last Saturday of October)<br>If the times here are wrong you can go to <a style="font-size:16px;text-decoration:none;" href=http://pgoh13.free.fr/mycourse2/actual_time_winter.php> this page</a> for winter time (from the last Sunday of October to the last Saturday of March the following year).<b><br></td></tr>
...
echo '<em>It is now ' . $heure . ' (' . $jour . ') in Hawaii (France -12 hours)</em>';
echo '<br>';
...
echo '<em>It is now ' . $heure . ' (' . $jour . ') in New Zealand (France +10 hours)</em>';
echo "<br>";
...
<?php
$heure= strftime ("%H:%M",time()+13500);
$jour= strftime ("%A",time()+13500);
echo '<em>It is now ' . $heure . ' (' . $jour . ') in Nepal (France +3 3/4 hours)</em>';
echo "<br>";
...
echo ' <em>It is now ' . $heure . ' (' . $jour . ') in Malaysia (France +6 hours)</em>';
echo "<br>";
...
<?php
$heure= strftime ("%H:%M",time()+12600);
$jour= strftime ("%A",time()+12600);
echo '<em>It is now ' . $heure . ' (' . $jour . ') in India (France +3.5 hours)</em>';
echo '<br>';
...
<p style="max-width:400px;">NOTE: <span style="font-size:smaller;font-style:italic;">All the times here are based on summer time in France. They will not be correct when winter time begins on the last Sunday of October each year in France. It is possible for me to have the times right throughout the year but this is possible only with php version 5 and above. Unfortunately my server at free.fr has only php version 4 at the moment.
...
<table style="max-width:400px;background-color:white"><tr><td><span style="font-size:0.8em;"><b>A quick revision:</b> hari Ahad (Sunday), Isnin (Monday), Selasa (Tuesday), Rabu (Wednesday), Khamis (Thursday), Jumaat (Friday), Sabtu (Saturday)
<br><br>
...
$day= strftime ("%A",time()+43200);
echo " <em>It is ".$day." in New Zealand</em>";
echo "<br>";
...
<br>
<p style="max-width:400px;font-style:italic;">(If <font color=red>semalam</font> is used for "yesterday" in the State where you are, replace <font color=red>kelmarin</font> below with <font color=red>semalam</font>). Incidentally where <font color=red>semalam</font> is used for "yesterday", last night would be <font color=red>malam semalam</font>.</p> 

<p>
<?
$today= strftime ("%A");
echo " Today is ".$today." in France";
