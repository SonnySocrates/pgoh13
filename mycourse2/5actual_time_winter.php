...
<script>
    function afficherDateHeure() {
        ...
        const nepalDay = jours[nepalTime.getDay()];
        const nepalHeures = nepalTime.getHours().toString().padStart(2, '0');
        const nepalMinutes = nepalTime.getMinutes().toString().padStart(2, '0');
        const nepalJourMois = nepalTime.getDate();
        const nepalNomMois = mois[nepalTime.getMonth()];
        const nepalAnnee = nepalTime.getFullYear();

        const malaysiaDay = jours[malaysiaTime.getDay()];
        ...
        const malaysiaJourMois = malaysiaTime.getDate();
        const malaysiaNomMois = mois[malaysiaTime.getMonth()];
        const malaysiaAnnee = malaysiaTime.getFullYear();

        const londonDay = jours[londonTime.getDay()];
        ...
        const londonJourMois = londonTime.getDate();
        const londonNomMois = mois[londonTime.getMonth()];
        const londonAnnee = londonTime.getFullYear();

        const indiaDay = jours[indiaTime.getDay()];
        const indiaHeures = indiaTime.getHours().toString().padStart(2, '0');
        const indiaMinutes = indiaTime.getMinutes().toString().padStart(2, '0');
        const indiaJourMois = indiaTime.getDate();
        const indiaNomMois = mois[indiaTime.getMonth()];
        const indiaAnnee = indiaTime.getFullYear();

        // Affichage
        const dateHeure = `
        Le jour, la date et l'heure en France sont : ${jourSemaine} ${jourMois} ${nomMois} ${annee}, ${heuresFrance}:${minutesFrance}.
        <br><br>
        Au Népal, il est ${nepalHeures}:${nepalMinutes} le ${nepalDay} ${nepalJourMois} ${nepalNomMois} ${nepalAnnee}.
        <br><br>
        En Malaisie, il est ${malaysiaHeures}:${malaysiaMinutes} le ${malaysiaDay} ${malaysiaJourMois} ${malaysiaNomMois} ${malaysiaAnnee}.
        <br><br>
        À Londres, il est ${londonHeures}:${londonMinutes} le ${londonDay} ${londonJourMois} ${londonNomMois} ${londonAnnee}.
        <br><br>
        En Inde, il est ${indiaHeures}:${indiaMinutes} le ${indiaDay} ${indiaJourMois} ${indiaNomMois} ${indiaAnnee}.
        `;
...
<hr>
<p>The time here will stay fixed. This is because by the time you come up with the answer the time might have changed!</p>
<?
